import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../account/current_account.dart';
import '../account/current_account_provider.dart';
import '../auth/auth_session.dart';
import '../financial_accounts/financial_account_providers.dart';
import '../opening_balances/opening_balance_providers.dart';
import 'account_transfer_models.dart';
import 'account_transfer_repository.dart';

final accountTransferRepositoryProvider = Provider<AccountTransferRepository>(
  (ref) => AccountTransferRepository(),
);

final ownerAccountTransferAccessProvider = Provider<bool>(
  (ref) =>
      ref.watch(currentAccountProvider).routeTarget ==
      TrustedAccountRouteTarget.staff,
);

final accountTransferListProvider =
    AsyncNotifierProvider<AccountTransferListController, List<AccountTransfer>>(
      AccountTransferListController.new,
    );

final accountTransferDetailProvider =
    NotifierProvider.family<
      AccountTransferDetailController,
      AsyncValue<AccountTransfer>,
      String
    >((id) => AccountTransferDetailController(id));

class AccountTransferListController
    extends AsyncNotifier<List<AccountTransfer>> {
  var _generation = 0;
  String? _authUserId;
  @override
  Future<List<AccountTransfer>> build() async {
    _watchIsolation();
    return const [];
  }

  Future<void> load() async {
    if (!_hasAccess()) {
      state = const AsyncData([]);
      return;
    }
    final session = ref.read(authSessionProvider);
    _authUserId = session.authUserId;
    final generation = ++_generation;
    state = const AsyncLoading();
    state = await AsyncValue.guard(() async {
      final items = await ref.read(accountTransferRepositoryProvider).list();
      return _canApply(generation, session.authUserId)
          ? items
          : state.value ?? [];
    });
  }

  Future<void> refresh() => load();

  void _watchIsolation() {
    final session = ref.watch(authSessionProvider);
    final hasAccess = ref.watch(ownerAccountTransferAccessProvider);
    if (session.status != AuthSessionStatus.authenticated || !hasAccess) {
      _generation++;
      _authUserId = null;
    }
  }

  bool _hasAccess() =>
      ref.read(authSessionProvider).status == AuthSessionStatus.authenticated &&
      ref.read(ownerAccountTransferAccessProvider);
  bool _canApply(int generation, String? authUserId) =>
      ref.mounted &&
      generation == _generation &&
      _hasAccess() &&
      _authUserId == authUserId &&
      ref.read(authSessionProvider).authUserId == authUserId;
}

class AccountTransferDetailController
    extends Notifier<AsyncValue<AccountTransfer>> {
  AccountTransferDetailController(this._financialEventId);
  final String _financialEventId;
  var _generation = 0;

  @override
  AsyncValue<AccountTransfer> build() {
    if (!ref.watch(ownerAccountTransferAccessProvider)) {
      return const AsyncError(
        AccountTransferFailure(
          'Account transfers require an active Owner/Admin session.',
        ),
        StackTrace.empty,
      );
    }
    Future.microtask(reload);
    return const AsyncLoading();
  }

  Future<bool> createOrUpdate(AccountTransferDraft draft) async {
    final current = state.value;
    try {
      if (current == null) {
        final result = await ref
            .read(accountTransferRepositoryProvider)
            .create(draft);
        await ref.read(accountTransferListProvider.notifier).refresh();
        state = AsyncData(
          await ref
              .read(accountTransferRepositoryProvider)
              .detail(result.financialEventId),
        );
      } else {
        await ref
            .read(accountTransferRepositoryProvider)
            .update(
              financialEventId: current.financialEventId,
              expectedVersionNumber: current.versionNumber,
              draft: draft,
            );
        await reload();
      }
      return true;
    } catch (error, stack) {
      state = AsyncError(error, stack);
      return false;
    }
  }

  Future<bool> submit() => _mutate(
    (item) => ref
        .read(accountTransferRepositoryProvider)
        .submit(item.financialEventId, item.versionNumber),
  );
  Future<bool> approve() => _mutate(
    (item) => ref
        .read(accountTransferRepositoryProvider)
        .approve(item.financialEventId, item.versionNumber),
  );
  Future<bool> reject(String reason) => _mutate(
    (item) => ref
        .read(accountTransferRepositoryProvider)
        .reject(item.financialEventId, item.versionNumber, reason),
  );

  Future<void> reload() async {
    final generation = ++_generation;
    state = const AsyncLoading();
    state = await AsyncValue.guard(() async {
      final item = await ref
          .read(accountTransferRepositoryProvider)
          .detail(_financialEventId);
      return ref.mounted && generation == _generation ? item : state.value!;
    });
  }

  Future<bool> _mutate(
    Future<AccountTransferMutationResult> Function(AccountTransfer item) action,
  ) async {
    final current = state.value;
    if (current == null) return false;
    try {
      await action(current);
      await reload();
      await ref.read(accountTransferListProvider.notifier).refresh();
      await ref.read(financialApprovalQueueProvider.notifier).load();
      ref.invalidate(financialAccountListProvider);
      return true;
    } catch (error, stack) {
      state = AsyncError(error, stack);
      return false;
    }
  }
}
