...
Package 12.2 adds the private Supabase Storage foundation and temporary upload reservation flow only. It creates the private `documents-private` bucket, an `app.document_upload_status` enum, and `app.document_uploads` for temporary unfinalized uploads. The package does not add pending states to `app.document_status`.

Temporary upload object keys use:

`temporary/<upload_uuid>/<cryptographically-random-token>`

The future finalized namespace is reserved as:

`objects/<document_uuid>/<cryptographically-random-token>`

Object keys are generated by trusted backend code and must not contain original filenames, document numbers, Client or Project names/IDs, email addresses, MIME types, or user-controlled path components.

Accepted upload types are limited to:

- `.pdf` with `application/pdf`
- `.jpg` and `.jpeg` with `image/jpeg`
- `.png` with `image/png`
- `.webp` with `image/webp`

The global maximum is `25 MiB` / `26,214,400` bytes. Zero-byte files, oversized files, unsupported extensions, unsupported MIME types, MIME/extension mismatches, dangerous executable/script/archive/macro-enabled extensions, dangerous double extensions, path traversal, malformed supported signatures, and caller metadata tampering are rejected. Frontend-supplied MIME, size and SHA-256 are advisory only; upload completion downloads the bounded private object through the trusted backend, verifies magic bytes, derives the verified MIME family, calculates SHA-256, and records verified facts.

Successful completion stops at `AWAITING_SCAN`. Package 12.2 does not insert newly uploaded files into `app.documents`, does not publish them as `ACTIVE`, does not make them Client-visible, and does not make them downloadable through normal document access. There is no scanner bypass. Package 12.3 remains responsible for the ClamAV-compatible scan/quarantine/final publication gate after the production scanner interface is selected.

Owner/Admin may authorize uploads. Client, Project Manager, Accountant and Site Supervisor uploads remain denied. Existing finalized `app.documents` access is re-authorized through a backend document-access gateway. Owner/Admin may access `ACTIVE` and `ARCHIVED` finalized documents for operational and historical/audit use. Clients may access only `ACTIVE`, `client_visible` finalized documents linked to their own Client/Project context; cross-Client, contractor-private and archived Client access is denied.

The document-access backend proxies authorized finalized file bytes with safe `Content-Type`, sanitized `Content-Disposition`, and `Cache-Control: no-store`. It must not return reusable raw Storage signed URLs for Client delivery. Any internal Storage authorization must never be stored, logged, or exposed as folder-wide access.

Physical deletion is permitted only for expired, failed, confirmed orphan, or future malicious/quarantined temporary objects under approved cleanup policy. Finalized business documents and their objects are retained; valid business records are not permanently deleted in MVP.
...
- Schema: `app.currency_exchanges` has exactly `id`, `financial_event_id`, `source_account_id`, `destination_account_id`, `source_amount`, `source_currency_code`, `destination_amount`, `destination_currency_code`, `exchange_rate_id`, `rate_base_currency_code`, `rate_quote_currency_code`, `rate_value`, `rate_source`, `fee_amount`, `fee_currency_code`, `fee_account_id`, `exchange_date`, `rounding_result`, and `reference`.
- Owner wrappers: `public.server_owner_create_currency_exchange`, `public.server_owner_update_currency_exchange`, `public.server_owner_submit_currency_exchange`, `public.server_owner_reject_currency_exchange`, `public.server_owner_approve_currency_exchange`, `public.server_owner_currency_exchange_list`, and `public.server_owner_currency_exchange_detail`.
- Scope: optional Project association. When a Project is supplied, the parent financial event stores the Project and derived Client, the transaction stores the Project reporting currency, and every ledger line receives the Project and Client IDs. Without a Project, parent and ledger Project/Client IDs remain `NULL` and the transaction stores contractor default reporting currency.
- Business rate: selected manual exchange-rate rows must match the exchange date, cover the source/destination pair, and use `1 base currency = rate_value quote currency`. Destination amount is server-derived only: source-as-base multiplies, source-as-quote divides. Calculation uses exact numeric arithmetic and ROUND_HALF_UP to destination currency decimal digits.
- Rounding: `rounding_result` is signed destination currency amount equal to rounded destination amount minus exact unrounded destination amount. It must stay within one half of the smallest destination-currency unit and is stored on the destination FX-clearing ledger line's existing `rounding_adjustment` field. No rounding control account or extra rounding ledger rows are added.
- Fees: zero fee stores `fee_currency_code` and `fee_account_id` as `NULL`. Positive fees use the supplied fee account or default to the source account, store the effective fee account, require an active unarchived fee account with active asset ledger, and require the fee currency to equal the fee account currency and be either source or destination currency.
- Posting: different-Owner approval atomically posts exactly four conversion lines without a fee: debit destination financial asset, credit destination-currency `CTRL-FX-CLEARING-<currency_code>`, debit source-currency `CTRL-FX-CLEARING-<currency_code>`, and credit source financial asset. With a fee, approval posts exactly two additional lines: debit `CTRL-FX-FEE-<currency_code>` and credit the fee-paying financial asset. FX clearing and fee controls are private active system `CONTROL` accounts, no linked financial account, debit normal side.
- Reporting: the four conversion lines use one common reporting value. If reporting is source currency, it is source amount; if reporting is destination currency, it is destination amount; otherwise a manual transaction-date source-to-reporting rate is required. Destination-currency lines use an immutable derived destination-to-reporting snapshot where needed with `rate_source = DERIVED_FROM_CURRENCY_EXCHANGE`. Fee lines use their own same-currency or manual transaction-date reporting snapshot.
- Balance effect: source decreases by source amount, destination increases by destination amount, fee account decreases by fee amount when present, unrelated accounts remain unchanged, conversion reporting effect nets to zero, and Project-associated fees carry Project/Client IDs. Negative source and fee-paying account balances remain permitted; balances remain derived from posted ledger entries.
