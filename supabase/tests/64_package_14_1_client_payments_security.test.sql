...
SELECT is_empty($$ SELECT table_name FROM information_schema.tables WHERE table_schema='app' AND table_name IN ('refunds','payment_uploads','payment_evidence') $$, 'excluded finance workflow tables absent except approved request, matching, expense, transfer and currency exchange packages');
