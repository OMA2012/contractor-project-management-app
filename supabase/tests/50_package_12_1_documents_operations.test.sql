...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('project_expenses','currency_exchanges','document_scans','document_thumbnails')), 'finance scanner thumbnail tables absent except Package 14.1 client payments and Package 14.2 payment requests');
