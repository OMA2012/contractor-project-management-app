...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('client_payments','payment_requests','project_expenses','currency_exchanges','document_scans','document_thumbnails')), 'finance scanner thumbnail tables absent');
