...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('storage_buckets','document_scans','document_thumbnails','currency_exchanges')), 'storage scanner thumbnail finance objects absent except approved payment, request, matching and expense packages');
