...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('notifications','financial_transactions','ledger_entries','documents')), 'notification and later objects remain absent');
