...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='app' AND table_name IN ('payments','payment_requests','payment_matches','expenses','project_expenses','transfers','account_transfers','currency_exchanges','refunds','account_balances')), 'excluded workflow tables remain absent');
