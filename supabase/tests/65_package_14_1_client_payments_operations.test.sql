...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='app' AND table_name IN ('refunds','payment_uploads','payment_evidence')), 'excluded workflow tables remain absent except approved request, matching, expense, transfer and currency exchange packages');
