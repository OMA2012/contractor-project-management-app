...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='app' AND table_name IN ('account_transfers','currency_exchanges','refunds','payment_uploads','payment_evidence')), 'excluded workflow tables remain absent except approved request, matching and expense packages');
