...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('notification_delivery_attempts','notification_preferences','notification_retry_queue','documents','financial_transactions','ledger_entries')), 'delivery queues, preferences, documents and finance absent');
