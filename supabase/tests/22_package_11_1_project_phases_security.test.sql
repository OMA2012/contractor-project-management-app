...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('project_tasks','task_assignments','task_updates','financial_transactions','ledger_entries','documents')), 'task, finance and document tables remain absent');
