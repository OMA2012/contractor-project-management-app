...
SELECT lives_ok($$ SELECT * FROM public.server_owner_update_payment_request('00000000-0000-0000-0000-000000006801',(SELECT id FROM app.payment_requests WHERE description='First invoice'),1,(SELECT id FROM app.projects WHERE name='Project A 68'),120,'USD',DATE '2026-07-30',app.contractor_local_date() + 30,'Changed invoice') $$, 'Owner updates draft');
SELECT results_eq($$ SELECT requested_amount, due_date, description, version_number FROM app.payment_requests WHERE description='Changed invoice' $$, $$ VALUES (120::numeric, app.contractor_local_date() + 30, 'Changed invoice'::text, 2) $$, 'draft update changes allowed facts and increments version once');
