...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('project_phases','project_milestones','project_tasks','task_assignments','financial_transactions','ledger_entries','documents')), 'Stage 11 and finance/document tables remain absent');
