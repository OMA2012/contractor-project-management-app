...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('task_assignments','task_updates','progress_updates','completion_overrides','financial_transactions','ledger_entries','documents')), 'task-assignment, task-update, progress, finance and document tables remain absent');
