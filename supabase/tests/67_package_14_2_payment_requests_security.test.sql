...
SELECT isnt_empty($$ SELECT routine_name FROM information_schema.routines WHERE routine_schema='public' AND routine_name IN ('server_owner_create_payment_match','server_owner_update_payment_match','server_owner_approve_payment_match','server_owner_void_payment_match','server_owner_payment_match_list','server_owner_payment_match_detail') $$, 'approved Package 14.3 payment matching gateways exist');
...
SELECT is_empty($$ SELECT table_name FROM information_schema.tables WHERE table_schema='app' AND table_name IN ('refunds','payment_uploads','payment_evidence') $$, 'excluded finance workflow tables remain absent except approved payment matches, project expenses, account transfers and currency exchanges');
SELECT ok(EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE table_schema='app' AND table_name='document_links' AND constraint_name='document_links_payment_request_fk'), 'Package 12.4 activates document payment request links');
