...
SELECT is_empty($$ SELECT table_name FROM information_schema.tables WHERE table_schema='app' AND table_name IN ('account_transfers','currency_exchanges','refunds','payment_uploads','payment_evidence') $$, 'excluded finance workflow tables remain absent except approved payment matches and project expenses');
