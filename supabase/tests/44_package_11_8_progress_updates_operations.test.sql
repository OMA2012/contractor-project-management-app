...
SELECT ok(NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'app' AND table_name IN ('notifications','documents','document_links','financial_transactions','ledger_entries')), 'no notifications, documents or finance created');
