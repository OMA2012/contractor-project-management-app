...
It now includes the Stage 14 Package 14.1 database foundation for Client Payments, the Stage 14 Package 14.2 Payment Request foundation, the Stage 14 Package 14.3 Payment Matching foundation, and the Stage 15 Package 15.1 Project Expense foundation. Package 14.1 uses deterministic `CTRL-CLIENT-PAYMENT-<currency_code>` control accounts and is the ledger/account posting layer. Package 14.2 adds payment requests through the exact approved `app.payment_request_status` enum, exact 20-column `app.payment_requests` table, `PREQ-000001` global request numbering, Owner/Admin request workflows and current-Client safe request reads. Package 14.3 adds exact `app.payment_match_status`, exact 14-column `app.payment_matches`, Owner/Admin draft/update/different-Owner approval/void workflows, reversal-aware payment availability, request paid/remaining aggregation, synchronized request statuses, Client-safe derived balances, RLS, grants and tests. Package 15.1 adds the approved 12 `app.expense_categories`, immutable `code` with Owner-editable name/description, exact `app.project_expenses`, global `EXP-000001` numbering, Owner draft/update/submit/reject/list/detail workflows, different-Owner approval, duplicate protection, and atomic two-line posting that debits `CTRL-PROJECT-EXPENSE-<currency_code>` and credits the paying financial asset. Payment matching is allocation/reporting only and creates no financial events, financial transactions, ledger entries, exchange rates, account movements or Project receipt totals. It does **not** implement refunds, transfers, currency-exchange business workflows, file upload, transfer-evidence storage, document-finance link activation, finance notifications, Flutter finance screens, Edge Functions, Accountant activation, arbitrary journal entries, editable balances, partial reversals, payroll, budgets, tax processing, or financial reports.
